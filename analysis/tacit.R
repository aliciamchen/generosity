...
write.csv(d, here('data/tacit_tidy_data.csv'), row.names = FALSE)

# Deal with demographics csv
d.demographics <- read.csv(here('data/tacit_demographics.csv'))
...
mod <- glmer(data = d, 
             strategy ~ first_actual_higher + (1 | subject_id) + (1 | story),
             family =  'binomial'
)
