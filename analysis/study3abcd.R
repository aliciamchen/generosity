...
# Compare with study 1c (TODO for later: load in data, change variable names)
...
d.fairness.with.1c <- 
  left_join(
    d.first.response, d.fairness.story,
    suffix = c("_1c", "_3"),
    by = (c("story"))
  )
...
f <-
  ggplot(d.fairness.with.1c %>% filter(altruistic_status_second != "equal" & altruistic_status_second != "just_met" ),
